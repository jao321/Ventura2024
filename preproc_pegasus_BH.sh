...
#SBATCH --output=/vols/opig/users/gervasio/BH_changeo.out
#SBATCH --error=/vols/opig/users/gervasio/BH_changeo.err

source /vols/opig/users/gervasio/local/bin/activate

dir="/vols/opig/users/gervasio/covidao/samples/BH"
...
do
   ...
   pasta="$entry"
	...
	DefineClones.py -d "$pasta"/"${sample[0]}"_db-pass.tsv --vf v_call --model ham --norm len --dist 0.1 --format airr --nproc 8 --outname "${sample[0]}" --outdir "$pasta"
